<div class="container">
	<div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Estatus &nbsp; • &nbsp; Formulario</div>
                <div class="card-body">
                	<div class="row mb-4 justify-content-center">
                		<div class="col-md-12">
                			<fieldset>
                				<legend>Instrucciones</legend>
                				<span>Ingresa la información correcta para añadir nuevo estatus al sistema.</span>
                			</fieldset>
                		</div>
                	</div>
                	<form @if(isset($estatus)) method="post" @else method="Post" @endif action="@if(isset($estatus)) {{ route('estatus.update', $estatus->Id_Estatus) }} @else {{ route('estatus.store') }} @endif ">
                		@csrf
	                		...
	                		<div class="col-md-5">
		                		<div class="row">
			                		...
			                		<div class="col-md-12 input-group input-group-sm mb-3">
			                			...
			                			<input id="nombre_estatus" class="form-control form-control-sm" type="text" name="nombre" placeholder="Nombre" @isset($estatus) value="{{ $estatus->Nombre }}" @endisset required>
			                		</div>
								</div>
								<div class="row">
			                		...
			                		<div class="col-md-12 input-group input-group-sm mb-3">
									  	...
									  	<textarea id="descripcion_estatus" class="form-control" name="descripcion" style="height: 80px; min-height: 80px; max-height: 80px;" placeholder="Descripción">@if(isset($estatus)){{ $estatus->Descripcion }}@endif</textarea>
									</div>
								</div>
								<div class="row justify-content-center">
									<div class="col-md-6">
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#nombre_estatus').keyup(function() {
			var estatus = $('#nombre_estatus').val();
			if(estatus.length >= 4 && estatus.length  < 16){
				$('#estatus_btn').attr('disabled', false);
			}else{
				$('#estatus_btn').attr('disabled', true);
			}
		...
		$('#nombre_estatus').focusout(function() {
			var estatus = $('#nombre_estatus').val();
			if(estatus.length > 0 && estatus.length  < 11){
				$('#estatus_btn').attr('disabled', false);
			}else{
				$('#estatus_btn').attr('disabled', true);
			}
